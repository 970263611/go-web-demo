...
func (mc *MenuCreate) SetNameId(s string) *MenuCreate {
...
func (mc *MenuCreate) check() error {
	...
	if _, ok := mc.mutation.NameId(); !ok {
		return &ValidationError{Name: "nameId", err: errors.New(`ent: missing required field "Menu.nameId"`)}
	}
	if _, ok := mc.mutation.ParentId(); !ok {
		return &ValidationError{Name: "parentId", err: errors.New(`ent: missing required field "Menu.parentId"`)}
	}
	return nil
}
...
func (mc *MenuCreate) createSpec() (*Menu, *sqlgraph.CreateSpec) {
	...
	if value, ok := mc.mutation.NameId(); ok {
		_spec.SetField(menu.FieldNameId, field.TypeString, value)
		_node.NameId = value
	}
